    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional METI outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes METI and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::METI_TO_METI) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a METI-to-METI transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::METI_ONLY;
        ...
        // If METI-to-METI fails, create a peg-out transaction
