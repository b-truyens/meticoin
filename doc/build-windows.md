...
Below are some notes on how to build Meticoin Core for Windows.

The options known to work for building Meticoin Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Meticoin Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Meticoin Core applications.
...
Note that for WSL the Meticoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/meticoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/meticoin-project/meticoin.git
    sudo chmod -R a+rw meticoin
...
way. This will install to `c:\workspace\meticoin`, for example:

    make install DESTDIR=/mnt/c/workspace/meticoin
...
causes two of the meticoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the meticoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the meticoin source code).
