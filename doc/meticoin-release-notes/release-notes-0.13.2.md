Meticoin Core version 0.13.2 is now available from:

  <https://download.meticoin.org/meticoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/meticoin-project/meticoin/issues>
...
updates will be released anymore. Without security updates, using a meticoin
...
ECDSA signatures inside Meticoin transactions now use validation using
...
Previous versions of Meticoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Meticoin Core was to raise the default minimum
relay fee.

Meticoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Meticoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running meticoind without having to do any manual configuration.
...
Meticoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Meticoin Core will relay transactions with
...
entirely in Meticoin Core 0.13, and it is at this time undecided whether the
...
Meticoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Meticoin Core automatically creates a hidden service to listen on, without
manual configuration. Meticoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Meticoin Core is listening, and
...
Meticoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Meticoin
Core to use floating fees. By default, Meticoin Core will use floating
fees.
...
(default: `0.0002` METI/kB).

At all times, Meticoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) METI.
Furthermore, Meticoin Core will never create transactions paying less than
...
`meticoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 223232 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 223232 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /meticoinrpc>
        ProxyPass http://127.0.0.1:23232/
        ProxyPassReverse http://127.0.0.1:23232/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass meticoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< meticoinrpc:<password>
    </Location>
...
Restarting meticoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in meticoin.conf
...
meticoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/meticoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Meticoin Core code base has
...
- `meticoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `meticoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Meticoin scripting system that in combination with
...
Meticoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Meticoin Core.
...
The two phases are distinct in the Meticoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Meticoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Meticoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Meticoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of meticoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of meticoins being spent is exactly
...
- **Increased security for multisig:** Meticoin addresses (both P2PKH addresses
  ...
  believed to be provided by Meticoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Meticoin's current ledger (called the UTXO set),
  ...
  note that Meticoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Meticoin users to individually opt-in to almost any change in the Meticoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Meticoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Meticoin
client to Meticoin Core 0.13.2 to signal for segwit support. When a
...
Since Meticoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `meticoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `meticoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
